<template>
  <div class="modal-card p-3 box-modal block-bg  rounded-lg">
    <header class="box-modal-header is-spaced text-white font-bold">
      <div class="box-modal-title text-white font-bold">{{ $t('approvalIsRequired') }}</div>
      <button type="button" class="delete" @click="$emit('close')" />
    </header>
    <div class="note text-white font-bold">
    ...
    <b-button type="btn-gradient is-fullwidth" :disabled="isApproveBtnAlreadyPressed" @click="_approve">
      {{ $t('enable') }}
    </b-button>
  </div>
</template>
