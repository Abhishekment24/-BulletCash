<template>
  <b-tab-item :label="$t('relayer')" value="relayer" header-class="withdrawal_settings_relayer_tab">
    <div class="field">
      <b-field :label="$t('relayer')" data-test="withdrawal_settings_relayer_dropdown">
        <b-dropdown v-model="selectedRelayer" expanded aria-role="list" @change="onChangeRelayer">
          ...
          <b-dropdown-item
            v-for="{ name, toronodoServiceFee } in relayers"
            v-show="!isLoadingRelayers"
            :key="name"
            :value="name"
            aria-role="listitem"
          >
            {{ getRelayerName({ name, toronodoServiceFee }) }}
          </b-dropdown-item>
      ...
      <div class="withdraw-data is-spaced">
        <div class="withdraw-data-item">
          {{ $t('relayerFee') }}
          <span> {{ relayer.toronodoServiceFee }}% </span>
        </div>
      </div>
    ...
    <WithdrawTotal
      ...
      :service-fee="relayer.toronodoServiceFee"
    />
...
export default {
  ...
  methods: {
    ...
    getRelayerName({ name, toronodoServiceFee }) {
      return `${name} - ${toronodoServiceFee}%`
    },
