<template>
  <b-tab-item :label="$t('deposit')" header-class="button_tab_deposit w-full  tabing  text-white w-2/4">
    <fieldset>
      <b-field :label="$t('token')" data-test="token_list_dropdown" class="pt-2">
    ...
    <div class="pt-4 pb-4">
      <b-button
        type="btn-gradient is-fullwidth"
  ...
  methods: {
    ...
    openDepositModal() {
      this.$store.dispatch('application/prepareDeposit', {
        prefix: `toronodo-${this.selectedToken}-${this.selectedAmount}-${this.netId}`
      })
